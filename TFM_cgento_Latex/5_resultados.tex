...
En la sección \ref{section:causes_attrition} se comentó que los hogares panel poseen experiencia previa sobre la encuesta que puede afectar a su participación en olas posteriores. Esta experiencia puede abarcar varias ediciones, pero también puede ser informativo observar datos sobre la ola más reciente. En este apartado se muestran dos breves piezas de análisis. Primero se analiza cómo fue la proporción de abandonos en 2020 para cuatro variables que dan información sobre la experiencia de los hogares en 2017, y a continuación se comenta un ajuste necesario en la duración de las entrevistas por la detección de valores atípicos.
...
Como las pantallas del CAPI suelen estar asociadas o a una única pregunta del cuestionario o a los textos de introducción de cada sección, y esa información está recogida en el fichero de paradata, se calcularon las duraciones en cada una de las nueve secciones de la EFF. La figura \ref{fig:fig4} contiene los gráficos box-plot de las duraciones de las 9 secciones del cuestionario de la EFF2017. Estos gráficos son muy útiles para poder detectar valores atípicos. En la figura \ref{fig:fig4} se ve que hay valores atípicos en todas las secciones, y que algunos son particularmente altos, especialmente en la sección 9. Se sospecha que los entrevistadores no cerraron bien la aplicación del ordenador o la tablet al tomar un descanso, al interrumpir una entrevista, o al terminarla.

Para tratar estos valores atípicos, se decide imputar las duraciones de cada sección cuyo valor está por encima del percentil 99.9 de la duración de dicha sección. Se utiliza el algoritmo kNN (\textit{k-nearest neighbors}) con k=5 y distancia euclídea. Las características utilizadas para la imputación son, para cada sección, el número de preguntas que se formulan una sola vez, el número total de preguntas formuladas, el número de veces que se vuelve a una pantalla anterior, el número de paradas, el número de preguntas categóricas con 1-4 opciones de respuesta, 5-9 opciones de respuesta o más de diez opciones de respuesta, y el tiempo pasado en preguntas monetarias. Entonces, la duración total de la entrevista se calcula como la suma de las duraciones de todas las secciones. Este procedimiento se implementa por separado para las duraciones de cada ola.
